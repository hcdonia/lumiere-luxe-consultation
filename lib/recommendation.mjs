// Core recommendation engine for the new-guest consultation. Used by BOTH:
//   - api/recommend.mjs       -> the results page the guest is redirected to
//   - api/form-submitted.mjs  -> the Jotform webhook that texts/emails the guest
//                                their recap and saves prep notes for Michelle
// Keeping it here means the AI prompt, flow detection, and widget wiring live in
// ONE place and the two paths can never drift apart.
//
// NOTE: this module drags in sharp/heic-convert/@anthropic-ai/sdk at import time.
// Do not import it from lightweight crons (that's what lib/jotform-contact.mjs
// is for).
...
- HARD RULE — The Mini Luxe is about 6 foils at the parting and sides, nothing more. It is a gray blend, tone-down, gloss, or face-frame session with a few foils as a finishing touch. It is NOT a highlight service. If the guest CAME IN WANTING HIGHLIGHTS — brightness through their hair, their first real set of highlights, a sun-kissed look — you must NOT recommend The Mini Luxe, even when they say "nothing crazy", "just a little", or "I am a first-timer". Recommend The Natural Luxe for subtle brightness or The New Luxe for noticeable brightness.
...
export function wantsHaircutOnly(submission) {
  const answers = submission.answers || {};
  for (const [, field] of Object.entries(answers)) {
...
// Read a hidden marker field (e.g. alreadybooked, recapsent) by its unique name.
// Returns { qid, value } or null if the field doesn't exist on the form.
export function hiddenField(submission, fieldName) {
  const answers = submission.answers || {};
  for (const [qid, field] of Object.entries(answers)) {
    if ((field.name || '').toLowerCase() === fieldName.toLowerCase()) {
      return { qid, value: String(field.answer ?? '').trim() };
    }
  }
  return null;
}
...
export function buildHaircutRecommendation(submission) {
...
// The exact object the model must return. Sent as a structured-output
// constraint so a prose reply is impossible. On 2026-09-08 the model answered
// "I need to ..." instead of JSON, JSON.parse threw, and the form-submitted
// webhook died mid-flight — the guest was never texted and Michelle never heard
// about her. The serviceKey enum also guarantees a real WIDGET_IDS match.
const RECOMMENDATION_FORMAT = {
  type: 'json_schema',
  schema: {
    type: 'object',
    properties: {
      serviceKey: { type: 'string', enum: ['new-luxe', 'natural-luxe', 'mini-luxe'] },
      serviceName: { type: 'string' },
      explanation: { type: 'string' },
      details: { type: 'string' },
      consultationSummary: { type: 'string' },
    },
    required: ['serviceKey', 'serviceName', 'explanation', 'details', 'consultationSummary'],
    additionalProperties: false,
  },
};

// Pull the recommendation object out of a finished message. Structured outputs
// rule out prose, but a response cut short by max_tokens still yields truncated
// JSON, so name that failure rather than raising a bare SyntaxError. The brace
// slice is belt-and-braces against stray fences or preamble.
function parseRecommendation(message) {
  if (message.stop_reason === 'max_tokens') {
    throw new Error('recommendation was truncated (hit max_tokens)');
  }
  const text = (message.content || [])
    ...
    .trim();
  if (!text) {
    throw new Error(`recommendation had no text block (stop_reason: ${message.stop_reason})`);
  }
  ...
  try {
    ...
    throw new Error(`recommendation was not valid JSON: ${text.slice(0, 200)}`);
  }
}

// Run the full AI recommendation (answers + photos) for a standard/color guest.
// Returns the complete recommendation object with widget/booking info attached.
export async function generateRecommendation(submission) {
  ...
  const message = await anthropic.messages.create({
    ...
    // The consultationSummary alone runs 4-6 detailed sentences; 1024 left no
    // headroom, and a truncated response is unparseable JSON.
    max_tokens: 2048,
    messages: [
      {
        role: 'user',
  ...
  const recommendation = parseRecommendation(message);
